...
from Config.Exceptions import BkdError


class BkdInitializer:
    ...
    def getBot(self) -> BkdBot:
        return self.__bot

    def __create_bot(self, willListen: bool) -> BkdBot:
        if willListen:
            prefix = self.__config.BOT_PREFIX
            bot = BkdBot(listingSlash=True,
    ...
    def __add_cogs(self, bot: Bot) -> None:
        try:
            ...
            if len(bot.cogs.keys()) != self.__getTotalCogs():
                print(cogsStatus)
                raise BkdError(message='Failed to load some Cog')

        except BkdError as e:
            print(f'[Error Loading BKD_MUSIC]')
            print(e)
