...
class ProcessCommandsExecutor:
    ...
    def __init__(self, bot: BkdBot, guildID: int) -> None:
        self.__bot = bot
    ...
    @classmethod
    async def sendNowPlayingToGuild(cls, bot: BkdBot, playlist: Playlist, channel: TextChannel, song: Song, guild: Guild) -> None:
    ...
    @classmethod
    def __getPlayerViewForGuild(cls, channel: TextChannel, guildID: int, bot: BkdBot) -> View:
        buttons = cls.__getPlayerButtonsForGuild(channel, guildID, bot)
        view = BasicView(bot, buttons)
        return view

    @classmethod
    def __getPlayerButtonsForGuild(cls, textChannel: TextChannel, guildID: int, bot: BkdBot) -> List[Button]:
