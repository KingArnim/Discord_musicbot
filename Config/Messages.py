...
class Messages(Singleton):
    def __init__(self) -> None:
        if not super().created:
            ...
            configs = VConfigs()
            self.STARTUP_MESSAGE = 'Starting BKD_MUSIC...'
            self.STARTUP_COMPLETE_MESSAGE = 'BKD_MUSIC is now operating.'
            ...
            self.LOOP_ALL_ON = f'{self.__emojis.ERROR} BKD_MUSIC is looping all songs, use {configs.BOT_PREFIX}loop off to disable this loop first'
            self.LOOP_ONE_ON = f'{self.__emojis.ERROR} BKD_MUSIC is looping one song, use {configs.BOT_PREFIX}loop off to disable this loop first'
            self.LOOP_ALL_ALREADY_ON = f'{self.__emojis.LOOP_ALL} BKD_MUSIC is already looping all songs'
            self.LOOP_ONE_ALREADY_ON = f'{self.__emojis.LOOP_ONE} BKD_MUSIC is already looping the current song'
